...
```{r surveyframe}
# read in the data
# NOTE: scripts 01_scrape_health_depts, 04_clean_restaurant_list_csvs, and 02_sampling_frame must have been run already
...
Indigenous groups are 4% of the population.

https://www.statcan.gc.ca/eng/dai/btd/othervisuals/other010
visible minorities are 25% of the population

https://www.ic.gc.ca/eic/site/061.nsf/eng/h_03115.html
1.4% of business owners are indigenous, and 12.2% are visible minorities.

Therefore, Indigenous people are 0.35 times as likely to own a business, and visible minorities are 0.49 as likely to own a business.

## Survey 1
```{r}
set.seed(19893)
survey_1 <- survey_1 %>%
  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ADD MINORITY DATA SIMULATION HERE LALA>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
```
...
The distribution rf(n, df1, df2, ncp) = rf(num_rest, 10, 5) gives an approximate shape. Using the f-distribution formula, we know the distribution has a mean of 5/3, so we would multiply by 41,975 to get the mean to 69,959.
...
There seems to be no significant difference in business losses for disabled owners [@aug20192020].
...
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<CHECK IF THIS WORKS ONCE MINORITY DATA IS ADDED AMY>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
survey_2 <-
  bind_rows(
    survey_2 %>%
      filter((minority == "yes" | indigenous == "yes") & group == "treatment") %>%
...
```{r}
survey_1 <- survey_1 %>%
  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<ADD EMPLOYMENT DATA SIMULATION HERE LALA>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
```
...
```{r}
num_cols_1 = number of columns in the tibble <<<<<<<<<<<<<<<<<<<<<<<SET AMY>>>>>>>>>>>>>>>>>>>>

set.seed(19893)
survey_1 <- survey_1 %>%
  mutate(nonresponse = sample(c("yes","partial","no"), size = num_total, prob = c(0.6, 0.01, 0.39), replace = TRUE)) %>%
  filter(nonresponse != "no") 

set.seed(19893)
survey_1 <- survey_1 %>%
  mutate(which_partial = sample(c(4:num_cols_1), size = first(count(survey_1)), replace = TRUE))
...
```{r}
num_cols_2 = number of columns in the tibble <<<<<<<<<<<<<<<<<<<<<<<SET AMY>>>>>>>>>>>>>>>>>>>>
...
set.seed(19894)
survey_2 <- survey_2 %>%
  mutate(which_partial = sample(c(4:num_cols_1), size = first(count(survey_2)), replace = TRUE))
