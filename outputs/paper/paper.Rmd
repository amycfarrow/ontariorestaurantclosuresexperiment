...
grid.arrange(p1, p2, ncol = 2, top = "Employment distributions were similar in the treatment and control samples")
```
...
grid.arrange(p1, p2, ncol = 2, top = "Employment distributions were lower for the treatment sample")
```
...
```{r servicetypeimpact, fig.cap = "Survey 2 revenue distributions for service types with means marked", fig.width=7, fig.height=5, echo = FALSE, message = FALSE, warning = FALSE, out.width = '70%', fig.align = "center"}
...
ggplot() +
  ...
  geom_errorbar(data = means, aes(x = service_type, y = mean, ymax = mean, ymin = mean, fill = group), position = "dodge") +
  labs(fill = NULL,
...
ggplot() +
  ...
  geom_errorbar(data = means, aes(x = minority, y = mean, ymax = mean, ymin = mean, fill = group), position = "dodge") +
  labs(fill = NULL,
       x = "Racial and ethnic minorities",
       title = "Minority-owned businesses were more heavily impacted",
       y = "Revenue (in thousands of Canadian dollars)") +
  theme_minimal()
```
